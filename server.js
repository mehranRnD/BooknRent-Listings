...
// Hostaway API details from .env
const HOSTAWAY_API_URL = process.env.HOSTAWAY_API_URL; // Base URL for listings
const AUTHORIZATION_TOKEN = process.env.AUTHORIZATION_TOKEN;

// Listing IDs and image URLs
const listingImages = {
  288675:
  ...
  288677:
    "https://hostaway-platform.s3.us-west-2.amazonaws.com/listing/80066-288677-StsfW4f-mqGXpQEy2h9wIHfTp3UAU-C6Pp2stm9cSXE-66da9fe8ce2ea-small",
  288678:
  ...
  305327:
    "https://hostaway-platform.s3.us-west-2.amazonaws.com/listing/80066-305327-oxL4nm19qp4utWic3BIsrCBip6RYQe63wj8Sns2ML-E-66d6b6d4e24e2-small",
  306032:
  ...
  323227:
    "https://bookingenginecdn.hostaway.com/listing/80066-323227-77seG-17GHwZCjZ8--LSlCtXQMdeuDE--4g0eLcLd9Yxs-6724938be93a6?width=1280&quality=70&format=webp&v=2",
  323229:
...
// Listing IDs you want to fetch data for
const listingIds = Object.keys(listingImages);

// Serve static files from the public directory (for HTML and CSS)
...
app.get("/api/listings", async (req, res) => {
  try {
    ...
    const responses = await Promise.all(listingPromises);
    const listingsData = responses.map((response) => {
      const data = response.data.result;
      const listingId = data.id;

      // Attach the correct image URL to each listing
      return {
        id: listingId,
        name: data.name,
        price: data.price,
        imageUrl: listingImages[listingId] || "https://via.placeholder.com/150",
      };
    });

    res.json(listingsData);
  } catch (error) {
    console.error("Error fetching data from Hostaway:", error);
    res.status(500).json({ error: "Error fetching data from Hostaway" });
  }
});

// Endpoint to fetch a single listing's details by ID
app.get("/api/listings/:id", async (req, res) => {
  const listingId = req.params.id;
  try {
    const response = await axios.get(`${HOSTAWAY_API_URL}/${listingId}`, {
      headers: {
        Authorization: AUTHORIZATION_TOKEN,
        "Cache-Control": "no-cache",
      },
    ...
    console.error(`Error fetching details for listing ID ${listingId}:`, error);
